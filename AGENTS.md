...
- Wispace API auth: header **`x-psid`** (PSID Messenger) + **`X-Internal-Key`** (`WISPACE_INTERNAL_KEY`); liên kết mapping qua **`POST WISPACE_API_VERIFY_MESSENGER_TOKEN`** body `{ token, psid }` khi `MESSENGER_LINK_MODE=token`.
...
| Deploy / CI / VPS path | `.github/workflows/deploy.yml`, `.github/workflows/sync-env.yml`, `docs/doppler-secrets.md` |
