...
| Gap | Trạng thái POC |
|-----|----------------|
| `POST /messenger/study-calendar/sync` | ✓ Đã implement — Wispace cần **wire** gọi sau đổi lịch |
| `GET UserCalendar` qua `WISPACE_API_USER_CALENDAR_URL` | ✓ Đã dùng khi sync |
| Auth endpoint sync / ops | ✗ Chưa — cần gateway production |
| Tên học viên cho LLM (`displayName`) | ✗ Mặc định `'bạn'` |
