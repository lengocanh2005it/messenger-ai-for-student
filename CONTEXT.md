...
_Avoid_: user ID without clear meaning — always write "WISPACE userId"
...
Outbox table for study reminder scheduling. Entity: `StudyReminderJobEntity`. States: `pending` → `processing` → `sent` / `failed` / `cancelled`.
...
_Avoid_: ambiguous "score" — always write "band" or "target band"
...
**FREE_FORM**:
Chat interaction type subject to rate limiting: user sends free-form text → bot replies via LLM. This is the only bucket with a quota.
...
_Avoid_: chat queue — the class name is `DebounceChatQueue`
...
_Avoid_: hardening phase — these are specific numbered items
...
**claim table**:
Shorthand for `scheduled_report_claims` — used in the leader election pattern.
_Avoid_: lock table
...
_Avoid_: bare interface — a port is specifically a DI token pair
